...
# ──────────────────────────────────────────────
# __________________ SCORE GEO (RAW + PCT 7J) __________________
# ──────────────────────────────────────────────
...
def _add_geo_scores(df: DataFrame) -> DataFrame:
    ...
    Ajoute 5 colonnes :
      ...
      - geo_score_raw
      - geo_score_pct7d : percentile rank (0..100) calculé SUR LES 7 DERNIERS JOURS (dataset), null sinon
    """
    ...
    # Percentile 7 jours (sur le dataset) :
    # - On prend la date max dans df
    # - On calcule le percentile rank (0..100) de geo_score_raw parmi les événements des 7 derniers jours
    # - On remet null pour les lignes plus anciennes
    max_day = df.agg(F.max("Day").alias("max_day")).collect()[0]["max_day"]
    if max_day is None:
        return df.withColumn("geo_score_pct7d", F.lit(None).cast(DoubleType()))

    start_day = F.date_sub(F.lit(max_day), 6)  # 7 jours = max_day, max_day-6

    df_last7 = df.filter(F.col("Day") >= start_day)

    w = Window.orderBy(F.col("geo_score_raw").asc())
    df_last7 = df_last7.withColumn("geo_score_pct7d", F.percent_rank().over(w) * F.lit(100.0))

    # Join back (GlobalEventID est unique après dropDuplicates)
    df = (
        df.alias("all")
        .join(
            df_last7.select("GlobalEventID", "geo_score_pct7d").alias("p"),
            on="GlobalEventID",
            how="left",
        )
    )
...
def _clean_dataframe(df: DataFrame) -> DataFrame:
    ...
    # ─────────────────────────────────────────────────────────────
    # __________________ AJOUT SCORES GEO (5 COLONNES) _____________
    # ─────────────────────────────────────────────────────────────
